  ...
  <div id="databox">
    <div class="dataheader">
      <div class="firstheader">
        <el-form ref="form" :model="dataForm" label-width="80px">
          <el-form-item label="用户名称">
            <el-select v-model="dataForm.id" placeholder="请选择活动区域" class="uuupppcc">
              <el-option
                :key="index"
      ...
      <div class="secondheader">
        <el-button @click.native.prevent="addData" type="primary">添加域名</el-button>
        <div class="msgheader">
      ...
      >
        ...
        <el-table-column align="right" label="地址">
          <template slot="header">
            <span class="spstyle" v-edithass="ceshi">编辑</span>
          </template>
...
export default {
  data() {
    ...
    return {
      ceshi: this.$store.getters.roleVal,
      title: "NEW",
      search: "",
  ...
  methods: {
    ...
    getAllUsers() {
      let newId = this.$store.getters.userid;
      this.$store
        .dispatch("datamg/getUserData", { userid: newId })
        .then(response => {
          this.allUser = response.data;
          if (response.data.length !== 0) {
            this.dataForm.id = parseInt(response.data[0].id);
          }
        });
    },
    // 获取所有目标
    getAllJumps() {
      ...
      this.currentUser = this.$store.getters.userid;
      const temp = {
        id: this.currentUser,
        pgObj: newObj,
